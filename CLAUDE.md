...
- **Telegram cannot colour text.** The HTML subset is b/i/u/s/code/pre/a — no colour attribute anywhere, which is why anything that needs colour or alignment (the status card, the stats table) is rendered as an image. The text that remains — captions, the alert fan-out block — carries colour only through the level emoji, and always names the level in words beside it.
...
- **The status card is a chart too.** `charts.status_card` draws the level bar (zero to that pollutant's *high* limit, warn marked), the sparkline and the trend; `reports.status_report()` returns a `Report` like any window. A stale sensor stays text — a dead reader must not wait on a render, and there is nothing to draw. Alert fan-out is also text: it goes to every subscriber, each with their own theme.
