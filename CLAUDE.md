...
- **Telegram cannot colour text.** The HTML subset is b/i/u/s/code/pre/a — no colour attribute anywhere. Emoji carry colour instead: the trend uses squares (🟥/🟩) because circles (🟢🟠🔴) are the air-quality level, and a red circle beside a value would read as "high" rather than "rising". The arrow repeats the direction, so colour never carries meaning alone.
